...
def single_sample(task,template, demonstrations, sample, tokenizer, max_length,generation=False, generation_with_gold=False, max_new_tokens=None):
    demo_prompts = [template.verbalize(sample, sample.correct_candidate).strip() for sample in demonstrations]
    demo_prompts = task.train_sep.join(demo_prompts).strip()
    encode_fn = template.encode #without label
    verbalize_fn = template.verbalize # verbalize label
    unverbalized_eval_prompt = encode_fn(sample).strip(' ')# question + so
    if not generation:
        verbalized_eval_prompts = [verbalize_fn(sample, cand).strip(' ') for cand in sample.candidates]
        ...
        if len(demonstrations)>0:
            final_prompts = [(demo_prompts + task.train_sep + eval_prompt).lstrip().strip(' ') for eval_prompt in verbalized_eval_prompts]
        else:
            final_prompts = verbalized_eval_prompts
    else:
        if generation_with_gold:
            verbalized_eval_prompts = [verbalize_fn(sample, sample.correct_candidate)]
            unverbalized_eval_prompt_length = len(tokenizer.encode(unverbalized_eval_prompt))
            option_lens = [(len(tokenizer.encode(verbalized_eval_prompt)) - unverbalized_eval_prompt_length) for verbalized_eval_prompt in verbalized_eval_prompts]
            final_prompts = [(demo_prompts + task.train_sep + eval_prompt).lstrip().strip(' ') for eval_prompt in verbalized_eval_prompts] 
        else:
            option_lens = [0]
            final_prompts = [(demo_prompts + task.train_sep + unverbalized_eval_prompt).lstrip().strip(' ')]
...
def GLUEDataset(task,template, demonstrations, samples, tokenizer, max_length, train_as_classification=False,generation=False, generation_with_gold=False, max_new_tokens=None):
    data = []
    for sample in samples:
        encodings, option_lens = single_sample(task=task,
                                            template=template,
                                            ...
                                            generation_with_gold=generation_with_gold,
        ...
        if train_as_classification:
            data.append([{"input_ids": encodings[_i], "labels": correct_candidate_id, "option_len": option_lens[_i], "num_options": len(sample.candidates)} for _i in range(len(encodings))])
        else:
            data.append({"input_ids": encodings[correct_candidate_id], "labels": encodings[correct_candidate_id], "option_len": option_lens[correct_candidate_id],"num_options":1})
    return data

class HGDataset(torch.utils.data.Dataset):
    # longest first for batch finder
    def __init__(self, data_path, split, data_type,training_ratio=1.0):
        self.path = data_path
        if split == 'valid' and data_type in [TASK_TYPE.BOOLQ,TASK_TYPE.META_MATH]:
            load_split = 'test'
        else:
            load_split = split
        if data_type in [TASK_TYPE.BOOLQ, TASK_TYPE.COMMONSENSE, TASK_TYPE.CONVAI2]:
            ...
            dataset=load_dataset(data_path,split=load_split)
        ...
        if data_type == TASK_TYPE.GENERATE_E2E:
            ...
            all_dialogs = [example['dialog'] for example in dataset]
            input_x = []
            target = []
        ...
        elif data_type in [TASK_TYPE.META_MATH]:
            ...
            input_x = [d['instruction'] for d in dataset]
            target = [d['output'] for d in dataset]
        elif data_type in [TASK_TYPE.MMLU]:
            input_x = []
            target = []
