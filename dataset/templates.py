...
class SGLUERTETemplate(Template):
    # From PromptSource 1
...
class MNLITemplate(Template):
    ...
    def encode(self, sample):
        premise = sample.data['premise']
        hypothesis = sample.data['hypothesis']
        return f"{premise}\nDoes this mean that \"{hypothesis}\" is true? Yes or Maybe or No?\n"
...
class MRPCTemplate(Template):
    verbalizer = {0: "No", 1: "Yes"}
    def encode(self, sample):
        premise = sample.data["sentence1"].strip()
        hypothesis=sample.data["sentence2"].strip()
        return f"{premise}\nDoes this mean that \"{hypothesis}\" is true? Yes or No?\n"
...
class QNLITemplate(Template):
    verbalizer = {0: "Yes", 1: "No"}
    def encode(self, sample):
        premise = sample.data["question"].strip()
        hypothesis=sample.data["sentence"].strip()
        return f"{premise}\nDoes this mean that \"{hypothesis}\" is true? Yes or No?\n"
...
class QQPTemplate(Template):
    verbalizer = {0: "No", 1: "Yes"}
    def encode(self, sample):
        premise = sample.data["question1"].strip()
        hypothesis=sample.data["question2"].strip()
        return f"{premise}\nDoes this mean that \"{hypothesis}\" is true? Yes or No?\n"
